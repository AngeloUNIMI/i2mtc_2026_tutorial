...
def conv_orth_dist(kernel, stride = 1, device='cuda'):
    ...
    temp = torch.eye(new_s*new_s*i_c).reshape((new_s*new_s*i_c, i_c, new_s,new_s)).to(device)
    ...
    return torch.norm( Vmat@torch.t(out) - torch.from_numpy(temp).float().to(device) )
    
def deconv_orth_dist(kernel, stride = 2, padding = 1, device='cuda'):
    [o_c, i_c, w, h] = kernel.shape
...
def orth_dist(mat, stride=None, device='cuda'):
