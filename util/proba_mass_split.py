...
def splitDBTrainValTest(indexes, folderOrig, folderDest,
                        fileNameVec, extNew, dLabel, folds):
    ...
    for type in types:
        ...
        for foldInd in folds[type]:
            ...
            for indexOne in indexesFold:
                # read name
                pre, ext = os.path.splitext(fileNameVec[indexOne])
                # newname
                newName = pre + '.' + extNew
                shutil.copyfile(os.path.join(folderOrig, newName),
                                os.path.join(folderDest, type, dLabel, newName))
